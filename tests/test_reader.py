...
def test_discard_burn_in(root):
    ...
    # for 2 chains of length 500
    mcmc0 = read_chains('./tests/example_data/' + root)
    assert isinstance(mcmc0, MCMCSamples)
    mcmc1 = read_chains('./tests/example_data/' + root, burn_in=500)
    assert isinstance(mcmc1, MCMCSamples)
    for key in ['x0', 'x1', 'x2', 'x3', 'x4']:
        if key in mcmc0:
            assert key in mcmc1
            assert_array_equal(mcmc0[key][500:1000], mcmc1[key][:500])
    mcmc1.plot_2d(['x0', 'x1', 'x2', 'x3', 'x4'])
    mcmc1.plot_1d(['x0', 'x1', 'x2', 'x3', 'x4'])

    mcmc1 = read_chains('./tests/example_data/' + root, burn_in=-500.1)
    assert isinstance(mcmc1, MCMCSamples)
    for key in ['x0', 'x1', 'x2', 'x3', 'x4']:
        if key in mcmc0:
            assert key in mcmc1
            assert_array_equal(mcmc0[key][-500:], mcmc1[key][500:])
    mcmc1.plot_2d(['x0', 'x1', 'x2', 'x3', 'x4'])
    mcmc1.plot_1d(['x0', 'x1', 'x2', 'x3', 'x4'])
