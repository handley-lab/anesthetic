...
def test_plot_2d_kinds():
    ...
    axes = ns.plot_2d(
        params,
        kind={"lower": "kde_2d", "diagonal": "kde_1d", "upper": "scatter_2d"},
    )
...
def test_plot_2d_kinds_multiple_calls():
    ...
    axes = ns.plot_2d(
        params,
        kind={"diagonal": "kde_1d", "lower": "kde_2d", "upper": "scatter_2d"},
    )
...
def test_plot_2d_legend():
    ...
    ns.plot_2d(
        axes, label="l1", kind=dict(diagonal="hist_1d", upper="scatter_2d")
    )
    mc.plot_2d(
        axes, label="l2", kind=dict(diagonal="hist_1d", upper="scatter_2d")
    )
...
@pytest.mark.parametrize(
    "kwargs",
    [
        dict(color="r", alpha=0.5, ls=":", lw=1),
...
@pytest.mark.parametrize(
    "kind", ["kde", "hist", "default", skipif_no_fastkde("fastkde")]
)
...
def test_mcmc_stats():
    ...
    assert mcmc_half.index.get_level_values(0)[ceil(n0 / 2)] == n0 + floor(
        n1 / 2
    )
    ...
    assert mcmc_new.index.get_level_values(0)[floor(n0 / 2)] == n0 + floor(
        n1 / 2
    )
    ...
    assert mcmc_new.index.get_level_values(0)[ceil(n0 / 5)] == n0 + ceil(
        n1 / 4
    )
...
def test_stats():
    ...
    assert_array_equal(
        pc.set_beta(0).get_weights(), pc.set_beta(0).get_weights()
    )


@pytest.mark.parametrize(
    "kind",
    ["kde", "hist", "kde_1d", "hist_1d", skipif_no_fastkde("fastkde_1d")],
)
def test_masking_1d(kind):
    pc = read_chains("./tests/example_data/pc")
    mask = pc["x0"].to_numpy() > 0
    with (
        pytest.warns(UserWarning) if kind in ["kde", "hist"] else nullcontext()
    ):
...
def test_weighted_merging():
    ...
    samples = merge_samples_weighted(
        [samples_1, samples_2], label="Merged label"
    )
    mean = samples.xtest.mean()
    assert np.isclose(
        mean, (mean1 * weight1 + mean2 * weight2) / (weight1 + weight2)
    )
...
def test_beta():
    ...
    assert prior.beta == 0
    assert_array_equal(
        prior.index.get_level_values("weights"), prior.get_weights()
    )
    assert pc.beta == 1
    assert_array_equal(pc.index.get_level_values("weights"), pc.get_weights())
    assert_array_almost_equal(
    ...
    for beta in np.linspace(0, 2, 10):
        ...
        assert pc.beta == beta
        assert_array_equal(
            pc.index.get_level_values("weights"), pc.get_weights()
        )
        assert not np.array_equal(
            pc.index.get_level_values("weights"), weights
        )
...
def test_groupby_stats():
    ...
    for chain in [1, 2]:
        ...
        assert_allclose(
            mcmc.loc[mask, params].mean(), chains.mean().loc[chain]
        )
        assert_allclose(mcmc.loc[mask, params].std(), chains.std().loc[chain])
        assert_allclose(
            mcmc.loc[mask, params].median(), chains.median().loc[chain]
        )
        assert_allclose(mcmc.loc[mask, params].var(), chains.var().loc[chain])
        assert_allclose(
            mcmc.loc[mask, params].kurt(), chains.kurt().loc[chain]
        )
        assert_allclose(
            mcmc.loc[mask, params].kurtosis(), chains.kurtosis().loc[chain]
        )
        assert_allclose(
            mcmc.loc[mask, params].skew(), chains.skew().loc[chain]
        )
        assert_allclose(mcmc.loc[mask, params].sem(), chains.sem().loc[chain])
        assert_allclose(mcmc.loc[mask, params].cov(), chains.cov().loc[chain])
        assert_allclose(
            mcmc.loc[mask, params].corr(), chains.corr().loc[chain]
        )
        assert_allclose(
            [1, 1], chains.corrwith(mcmc.loc[mask, params]).loc[chain]
        )
    ...
    for col in params:
        if "chain" not in col:
            for chain in [1, 2]:
                mask = (mcmc.chain == chain).to_numpy()
                assert_allclose(
                    mcmc.loc[mask, col].mean(), chains[col].mean().loc[chain]
                )
                assert_allclose(
                    mcmc.loc[mask, col].std(), chains[col].std().loc[chain]
                )
                assert_allclose(
                    mcmc.loc[mask, col].median(),
                    chains[col].median().loc[chain],
                )
                assert_allclose(
                    mcmc.loc[mask, col].var(), chains[col].var().loc[chain]
                )
                assert_allclose(
                    mcmc.loc[mask, col].kurt(), chains[col].kurt().loc[chain]
                )
                assert_allclose(
                    mcmc.loc[mask, col].kurtosis(),
                    chains[col].kurtosis().loc[chain],
                )
                assert_allclose(
                    mcmc.loc[mask, col].skew(), chains[col].skew().loc[chain]
                )
                assert_allclose(
                    mcmc.loc[mask, col].sem(), chains[col].sem().loc[chain]
                )
                assert_allclose(
                    mcmc.loc[mask, col].cov(mcmc.loc[mask, col]),
                    chains[col].cov(mcmc.loc[mask, col]).loc[chain],
                )
                assert_allclose(
                    mcmc.loc[mask, col].corr(mcmc.loc[mask, col]),
                    chains[col].corr(mcmc.loc[mask, col]).loc[chain],
                )
                q = np.random.rand()
                assert_allclose(
                    mcmc.loc[mask, col].quantile(q),
                ...
                assert_allclose(
                    mcmc.loc[mask, col].kurtosis(), group.kurtosis()
                )
...
def test_groupby_plots():
    ...
    for param in params:
        ...
        for chain in [1, 2]:
            ...
            mcmc_widths = [
                p.get_width() for ax in mcmc_axes for p in ax.patches
            ]
            gb_widths = [p.get_width() for ax in gb_axes for p in ax.patches]
            assert_allclose(mcmc_widths, gb_widths)

            mcmc_heights = [
                p.get_height() for ax in mcmc_axes for p in ax.patches
            ]
    ...
    for param in params:
        ...
        _, mcmc_ax = plt.subplots()
        for chain, gb_ax in zip([1, 2], gb_plots):
            mcmc_ax = mcmc.loc[mcmc.chain == chain][param].plot.kde(ax=mcmc_ax)
        [
