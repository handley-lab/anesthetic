...
def test_beta_zero_nan_handling():
    ...
    Addresses issue where logL=-inf * beta=0 = NaN, which should be 0.
    Tests both scalar beta=0 and array cases with beta=0 at different positions.
    """
    ns = read_chains("./tests/example_data/pc")
    ...
    assert not beta_logL_scalar.isna().any(), "Scalar beta=0 should not produce NaN"
    ...
    assert not beta_logL_first[0.0].isna().any(), "beta=0 column should not contain NaN"
    assert_array_equal(beta_logL_first[0.0], 0.0)
    
    # Test array with beta=0 at middle position  
    beta_array_middle = np.array([0.5, 0.0, 1.0])
    beta_logL_middle = test_ns._betalogL(beta=beta_array_middle)
    assert not beta_logL_middle[0.0].isna().any(), "beta=0 column should not contain NaN"
    assert_array_equal(beta_logL_middle[0.0], 0.0)
    ...
    assert not beta_logL_last[0.0].isna().any(), "beta=0 column should not contain NaN"
