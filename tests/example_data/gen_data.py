...
# MCMC
# ----
def mcmc_sim(ndims=Ndims):
    """ Simple Metropolis Hastings algorithm. """
    x = [np.array([0, 0, 0.1, 0.5, 3])]
    l0 = [loglikelihood(x[-1])]
    w = [1]

    while len(x) < 10000:
        x1 = x[-1] + np.random.randn(ndims)*0.1
        l1 = loglikelihood(x1)
        if np.random.rand() < np.exp(l1-l0[-1]):
            x.append(x1)
            l0.append(l1)
            w.append(1)
        else:
            w[-1] += 1
    return np.array(x), np.array(l0), np.array(w)
...
roots.append(root)
mcmc[['weight', 'minuslogL'] + columns][:len(mcmc)//2].to_csv(root + '_1.txt',
                                                              sep=' ',
                                                              index=False,
                                                              header=False)
mcmc[['weight', 'minuslogL'] + columns][len(mcmc)//2:].to_csv(root + '_2.txt',
                                                              sep=' ',
                                                              index=False,
                                                              header=False)
...
roots.append(root)
mcmc[['weight', 'minuslogL'] + columns].to_csv(root + '.txt',
                                               sep=' ',
                                               index=False,
                                               header=False)
...
def ns_sim(ndims=Ndims, nlive=125):
    ...
    for _ in tqdm.tqdm(range(nlive*11)):
        ...
        while live_likes[i] <= Lmin:
            live_points[i, :] = np.random.uniform(low, high, size=ndims)
            live_likes[i] = loglikelihood(live_points[i])
    return (np.array(dead_points),
            np.array(dead_likes),
...
ns[columns + ['logL', 'logL_birth']].to_csv(root + '_dead-birth.txt',
                                            sep=' ',
                                            index=False,
                                            header=False)
live_ns[columns + ['logL', 'logL_birth']].to_csv(root + '_phys_live-birth.txt',
                                                 sep=' ',
                                                 index=False,
                                                 header=False)
...
roots.append(root)
ns['logdX'] = ns.logdX()
ns[columns + ['logL', 'logL_birth', 'logdX', 'cluster']].to_csv(
...
ns[columns + ['logL', 'logdX', 'cluster']].to_csv(root + 'ev.dat',
                                                  sep=' ',
                                                  index=False,
                                                  header=False)
live_ns[columns + ['logL', 'cluster']].to_csv(root + 'phys_live.points',
                                              sep=' ',
                                              index=False,
                                              header=False)
...
ns[columns + ['logL', 'logdX', 'cluster']].to_csv(root + 'ev.dat',
                                                  ...
                                                  index=False,
                                                  header=False)
ns[columns + ['logL', 'cluster']].to_csv(root + 'phys_live.points',
                                         sep=' ',
                                         index=False,
                                         header=False)
