...
def get_logLmaxAB(meanA,meanB,covA,covB,logLmaxA=0, logLmaxB=0):
    ...
    logLmaxAB = -1/2 * meandiff@linalg.solve(covA+covB,meandiff)+logLmaxA+logLmaxB
    return logLmaxAB

def get_logR(meanA,meanB,covA,covB,V):
    meandiff = np.array(meanA)-np.array(meanB)
    logR = -1/2*meandiff@linalg.solve(covA+covB,meandiff)-1/2*np.linalg.slogdet(2*np.pi*(covA+covB))[1]+np.log(V)
    return logR
...
def test_tension_stats_compatiable_gaussian(samples_plot=True,stats_table=True, stats_plot=True, hist_plot=True):
    nlive = 1000
    bounds = [[0, 1], [0,1], [0, 1]]
...
def test_tension_stats_incompatiable_gaussian(samples_plot=True,stats_table=True, stats_plot=True, hist_plot=True):
    nlive = 1000
    bounds = [[0, 1], [0,1], [0, 1]]
...
test_tension_stats_compatiable_gaussian()
#test_tension_stats_incompatiable_gaussian()
# %%
